...
class Character:
    ...
    def attack(self, target):
        damage = max(1,self.atack_power - target.defense)
        target.take_damage(damage)
        print(f'{self.name} shoots {target.name} and deals {damage} damage!')

    def take_damage(self, amount):
        self.health -= amount
        print(f'{self.name} takes {amount} damage! health is reduced to {self.health}')
...
def battle (fighter1, fighter2):
    ...
    while fighter1.is_alive() and fighter2.is_aliver():
        attacker = fighters[0]
        defender = fighters[1]
